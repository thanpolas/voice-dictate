#!/usr/bin/env bash
# @fileoverview Configuration file writers. Generates the shell-side runtime
# config (bin/config.local.sh) sourced by dikta.sh and the Lua-side hotkey
# config (~/.hammerspoon/dikta-config.lua) loaded by the Hammerspoon
# module. Uses the : "${VAR:=…}" override pattern so pre-set env values still
# win over the persisted defaults.

set -euo pipefail

# Source lib.sh from the same directory so log() is available.
# shellcheck source=lib.sh
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

# Write the shell-side runtime config file sourced by dikta.sh.
# Re-runs are safe — the file is overwritten in full each time.
# $1 — destination path for the generated config (typically bin/config.local.sh).
# $2 — MODEL_PATH default to embed in the file.
# $3 — LANGUAGE default to embed in the file.
function write_shell_config() {
  local destination="${1}"
  local model_path="${2}"
  local language="${3}"
  cat > "${destination}" <<EOF
# Dikta shell config — generated by install.sh; gitignored.
# Edit to change defaults. Per-invocation env overrides still work:
#   LANGUAGE=en ./bin/dikta.sh smoke

: "\${MODEL_PATH:=${model_path}}"
: "\${LANGUAGE:=${language}}"
: "\${THREADS:=8}"
: "\${AUDIO_DEVICE:=:0}"
EOF
  log ok "wrote ${destination}"
}

# Write the Lua-side config file loaded by the Hammerspoon module on every reload.
# The file lives outside the repo so user edits survive a clean re-clone.
# stream_sh and server_sh are derived from dikta_sh — they always live in
# the same bin/ dir. ffmpeg_path is resolved at install time because Hammer-
# spoon launches from launchd with a minimal PATH and cannot look up the
# binary itself; the Homebrew prefix differs between Apple Silicon (/opt/
# homebrew) and Intel (/usr/local), so a literal in the Lua module would
# silently break the install on the other architecture.
# $1 — destination path (typically ~/.hammerspoon/dikta-config.lua).
# $2 — absolute path to bin/dikta.sh that the Lua module will invoke.
# $3 — absolute path to the ffmpeg binary discovered at install time.
function write_lua_config() {
  local destination="${1}"
  local dikta_sh="${2}"
  local ffmpeg_path="${3}"
  local bin_dir="${dikta_sh%/*}"
  mkdir -p "$(dirname "${destination}")"
  cat > "${destination}" <<EOF
-- Dikta Hammerspoon config — generated by install.sh.
-- Edit to change defaults, then run hs.reload() from the Hammerspoon Console.

return {
  dikta_sh = "${dikta_sh}",
  stream_sh = "${bin_dir}/stream.sh",
  server_sh = "${bin_dir}/stream-server.sh",
  ffmpeg_path = "${ffmpeg_path}",
  toggle_mods = {"cmd", "shift"},
  toggle_key = "D",
  right_alt_keycode = 61,
  hide_hammerspoon_icon = true,
}
EOF
  log ok "wrote ${destination}"
}
